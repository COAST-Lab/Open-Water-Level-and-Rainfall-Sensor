...
const char * eventName = "rainfall_0001";
...
void setup(void) {
  if (PUBLISHING==1) {
   ...
   delay(3000); // to see response from begin command

  Serial.begin(9600);
  Log.info("Maxbotix Test");

  //RainfallSensor Addition
  while (!Sensor.begin()) {
...
void loop(void) {
  ...
  case DATALOG_STATE: {
    ...
    snprintf(data, sizeof(data), "%li,%.5f,%.2f,%.02f,%.02f", //,%.5f,%.5f,%.5f,%.5f,%.5f,%.02f,%.02f",
      real_time, // if it takes a while to connect, this time could be offset from sensor recording
      range_cm,
      Sensor.getRainfall(1), //RainfallSensor Addition
      cellVoltage,
      stateOfCharge
    );
